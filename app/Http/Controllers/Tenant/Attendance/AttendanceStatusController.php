...
class AttendanceStatusController extends Controller
{
    ...
    public function updateCancelAPI(Request $request)
    {
        ...
        $status_id = resolve(StatusRepository::class)->$method();
        $details = AttendanceDetails::where('attendance_id',$request->attendance_id)->get();
        $details = $details->load('status', 'parentAttendanceDetails', 'attendance');



        if (!($request->get('status_name') == 'cancel' &&
            $details[0]->status->name == 'status_pending' && $details[0]->attendance->user->id == auth()->id())) {
            $this->departmentAuthentications($details[0]->attendance->user->id);
        }

        $query=DB::transaction(function () use ($details, $status_id, $request) {
            $this->service
                ->setDetails($details[0])
                ->setAttributes([
                    'status_id' => $status_id,
                    'requestedStatus' => $request->get('status_name'),
                    'previousStatus' => $details[0]->status->name,
                    'review_by' => auth()->id()
                ])
                ->setModel($details[0]->attendance->user)
                ->updateAttendanceDetailsStatus();
        });
