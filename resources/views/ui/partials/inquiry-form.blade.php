@php
    ...
    $idPrefix = 'inq-' . preg_replace('/[^a-z0-9]+/i', '-', (string) $source);
@endphp

<form class="{{ $formClass }}" action="{{ route('inquiry.store') }}" method="POST">
    @csrf
    <input type="hidden" name="source" value="{{ $source }}">
    <input type="hidden" name="form_started_at" id="{{ $idPrefix }}-started" value="">
    {{-- Honeypot: obscure name so password managers do not autofill. --}}
    <div class="sk-hp" aria-hidden="true" style="position:absolute;left:-9999px;top:auto;width:1px;height:1px;overflow:hidden;">
        <label for="{{ $idPrefix }}-sk-hp">Leave blank</label>
        <input type="text" id="{{ $idPrefix }}-sk-hp" name="sk_hp_field" value="" tabindex="-1" autocomplete="off">
    </div>
    <script>
      document.getElementById(@json($idPrefix . '-started')).value = String(Math.floor(Date.now() / 1000));
    </script>
    ...
    @if($showCompany)
        <div class="{{ $rowClass }}">
            <div class="{{ $fieldClass }}">
                ...
                <input type="text" id="{{ $idPrefix }}-company" name="company" placeholder="Your company" value="{{ old('company') }}">
            </div>
            <div class="{{ $fieldClass }}">
                <label for="{{ $idPrefix }}-name">Your Name</label>
                <input type="text" id="{{ $idPrefix }}-name" name="name" placeholder="Your name" value="{{ old('name') }}" required>
            </div>
        </div>
        <div class="{{ $rowClass }}">
            <div class="{{ $fieldClass }}">
                <label for="{{ $idPrefix }}-phone">Phone / WhatsApp</label>
                <input type="tel" id="{{ $idPrefix }}-phone" name="phone" placeholder="+971 __ ___ ____" value="{{ old('phone') }}" required>
            </div>
            <div class="{{ $fieldClass }}">
                <label for="{{ $idPrefix }}-email">Email</label>
                <input type="email" id="{{ $idPrefix }}-email" name="email" placeholder="you@company.com" value="{{ old('email') }}" required>
            </div>
        </div>
    @else
        @if($compact)
            <div class="{{ $rowClass }}">
                <div class="{{ $fieldClass }}">
                    <label for="{{ $idPrefix }}-name">Your Name</label>
                    <input type="text" id="{{ $idPrefix }}-name" name="name" placeholder="e.g. Ahmed Khan" value="{{ old('name') }}" required>
                </div>
                <div class="{{ $fieldClass }}">
                    <label for="{{ $idPrefix }}-email">Email</label>
                    <input type="email" id="{{ $idPrefix }}-email" name="email" placeholder="Enter email address" value="{{ old('email') }}" required>
                </div>
            </div>
            <div class="{{ $rowClass }}">
                <div class="{{ $fieldClass }}">
                ...
                @if($showStorageSelect)
                    <div class="{{ $fieldClass }}">
                        <label for="{{ $idPrefix }}-storage-type">Storage type</label>
                        <select id="{{ $idPrefix }}-storage-type" name="storage_type" required>
    ...
    @if($showSize)
        <div class="{{ $fieldClass }}">
            <label for="{{ $idPrefix }}-size">Unit size</label>
            <select id="{{ $idPrefix }}-size" name="size">
    ...
    @if($showStoring)
        <div class="{{ $rowClass }}">
            <div class="{{ $fieldClass }}">
                <label for="{{ $idPrefix }}-storing">What are you storing?</label>
                <select id="{{ $idPrefix }}-storing" name="storing">
            ...
            @if($showDuration)
                <div class="{{ $fieldClass }}">
                    <label for="{{ $idPrefix }}-duration">Approx. duration</label>
                    <select id="{{ $idPrefix }}-duration" name="duration">
            ...
            @if($showVolume)
                <div class="{{ $fieldClass }}">
                    <label for="whFormVolume">Approx. volume</label>
    ...
    @if($showMessage)
        <div class="{{ $fieldClass }}">
            ...
            <textarea id="{{ $idPrefix }}-message" name="message" rows="2" placeholder="Enter message">{{ old('message') }}</textarea>
        </div>
    @endif
