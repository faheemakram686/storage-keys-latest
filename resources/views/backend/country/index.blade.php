...
@section('content')
            ...
            <div class="nk-block-head">
                <div class="nk-block-between">
                    <div class="nk-block-head-content">
                        <h3 class="nk-block-title page-title">Countries List</h3>
    ...
    <div class="modal fade" tabindex="-1" role="dialog" id="editCountry" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title text-capitalize" id="ajax_model_title">Edit Country</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body" data-form="ajax-form-modal">
                    <form method="post" action="{{ url('admin/update-country') }}" id="updateCountryForm">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label> Name <span class="text-danger"></span></label>
                                    <input  type="hidden" name="country_id" placeholder="Name" required>
                                    <input class="form-control" type="text" name="edit_country_name" placeholder="Name" required>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        for (i = 0; i < data.length; i++) {
                            ...
                            ' </td>'+
                                '<td class="nk-tb-col nk-tb-col-tools" >'+
                            ' <span class="badge badge-success">'+data[i].is_defult+'</span>'+
                            ' </td>'+
                            '  <td class="nk-tb-col nk-tb-col-tools">'+
            ...
            $('#countryTable').on('click', '.btn-edit', function() {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        $('input[name=country_id]').val(id);
                        $('input[name=edit_country_name]').val(res.name);
                        // $('check-box[name=edit_is_defult]').val(res.name);
                        if (res.is_defult==1)
                        {
