...
@section('title', '| Move-Out Request')
@section('content')
            ...
            <div class="nk-block-head">
                <div class="nk-block-between">
                    <div class="nk-block-head-content">
                        <h3 class="nk-block-title page-title">Move-Out Items List</h3>
                        <div class="nk-block-des text-soft">
                            <h5 class="">{{$data['moveout'][0]->customer->customer_name}}</h5>
                            <h6 class="">{{$data['moveout'][0]->contract->subject}}</h6>
                        </div>
                    </div>
                    <div class="nk-block-head-content">
                            ...
                            <div class="toggle-expand-content" data-content="pageMenu">
                                <ul class="nk-block-tools g-3">
                                    <li class="nk-block-tools-opt">
                                        <a  href="{{url('admin/print-barcode-labels').'/'.$data['moveout'][0]->id}}" class="btn btn-primary btn-sm" ><em class="icon ni ni-printer-fill"></em><span>Print All</span></a>
                                    </li>
            ...
            <div class="card card-preview">
                <div class="card-inner">
                    <table class="datatable-init-export nk-tb-list nk-tb-ulist" data-auto-responsive="true"  id="datatable" >
                        ...
                        @foreach($data['moveout'][0]->contract->barcode as  $key => $barcode)
                            ...
                            <td class="nk-tb-col nk-tb-col-tools" >
                                <span class="{{(($barcode->status == 'Moved-Out')? 'badge badge-danger':'badge badge-success')}}">{{$barcode->status}}</span>
                                </td>
...
{{--                                                    <li><a href="{{url('admin/reprint-barcode-labels').'/'.$barcode->code}}" class="btn-label"  ><em class="icon ni ni-printer-fill"></em><span>Reprint Label</span></a></li>--}}
                                                    <li><a href="#" class="btn-edit" data='{{$barcode->id}}' data-toggle="modal" data-target="#editMoveout"><em class="icon ni ni-edit"></em><span>Edit</span></a></li>
    ...
    <div class="modal fade" tabindex="-1" role="dialog" data-backdrop="static" data-keyboard="false" id="editMoveout" aria-hidden="true">
        <div class="modal-dialog modal-sm" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title text-capitalize" id="ajax_model_title">Edit Move-Out</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body" data-form="ajax-form-modal">
                    <form method="post" action="{{ url('admin/update-move-out') }}" id="UpdateMoveOut">
                        @csrf
                        <div class="row">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#UpdateMoveOut').on('submit', function(e) {
                e.preventDefault();
                var formData=$('#UpdateMoveOut').serialize()
                $.ajax({
                    type: "get",
