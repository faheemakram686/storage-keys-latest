                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="customer_id" id="customer_id" class="form-control " data-live-search="true" required>
                                            ...
                                            @foreach( $data['customers'] as $customer)
                                                <option value="{{$customer->id}}">{{$customer->full_display_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-striped" id="dynamic_field">
                                                ...
                                                <tr>
                                                    <td colspan="4"></td>
                                                    <td colspan="2">VAT</td>
                                                    <td ><input class="form-control d-inline" type="number" step="any" id="vat" name="vat" value="5" min="0">%</td>
                                                </tr>
                                                <tr>
                                                    <td colspan="4"></td>
            ...
            function getOrders(customer_id) {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data.length > 0) {
                            for (i = 0; i < data.length; i++) {
                                html3 += '<option  value="' + data[i].id + '">Order No# ' + data[i].id + '</option>';
                            }
                        } else {
            ...
            function getEstimate(contract_id) {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (!units.length) {
                            // Backward compatible single-unit estimate
                            units = [{
                        ...
                        for (i = 0; i < units.length; i++) {
                            ...
                            var itemId = units[i].storage_unit_id || termLengthId;
