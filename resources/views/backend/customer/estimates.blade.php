                                ...
                                <div class="card-inner card-inner-lg">
                                    <div class="nk-block-head">
                                        <div class="nk-block-between d-flex justify-content-between">
                                            <div class="nk-block-head-content">
                                                <h4 class="nk-block-title">Estimates</h4>
                                            ...
                                            <div class="d-flex align-center">
                                                <div class="nk-tab-actions me-n1">
                                                    <a href="{{url('admin/create-customer-estimate')}}/{{$data['customer']->id}}" class="btn btn-primary btn-sm"  ><em class="icon ni ni-plus"></em><span>Add New Estimate</span></a>
                                                </div>
                                        ...
                                        <div class="card border border-light">
                                            <table class=" table table-md datatable-init-export nk-tb-list nk-tb-ulist" data-auto-responsive="true"  id="" >
                                                ...
                                                <tr class="nk-tb-item nk-tb-head">
                                                    ...
                                                    <th class="nk-tb-col"><span class="sub-text">Storage Unit</span></th>
...
{{--                        @isset($data)--}}
                            <div class="modal-body" data-form="ajax-form-modal">
                                <form method="post" action="{{ url('admin/save-contract') }}" id="ContractForm">
                                    @csrf
...
{{--                        @endisset--}}
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#countryTable').on('click', '.btn-contract', function() {
                var id = $(this).attr('data');
                $.ajax({
                    url: '{{ url('admin/get-estimate') }}',
                    type: 'get',
                    async: false,
                    dataType: 'json',
                    data: { id: id },
                    success: function(res) {
                        ...
                        $('input[name=customer_id]').val(res.estimate[0].customer_id);
                        $('input[name=estimate_id]').val(res.estimate[0].id);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        for (i = 0; i < data.length; i++) {
                            ...
                            var storageUnitPrice = data[i].unit_price || ((data[i].storageunit && data[i].storageunit.price) ? data[i].storageunit.price : 'N/A');

                            html += ' <tr class="nk-tb-item odd">'+
                                ' <td class="nk-tb-col nk-tb-col-tools sorting_1">'+c+'</td>'+
                                ' <td class="nk-tb-col nk-tb-col-tools"><a href={{url('admin/estimate/detail')}}/' + data[i].id + ' class="btn-edit" data='+data[i].id+'>' + storageUnitName + '/' + termLengthTitle + '</a></td>' +
                                ...
                                '  <td class="nk-tb-col nk-tb-col-tools">'+
                                ' <ul class="nk-tb-actions"  style="right: 1.0rem; !important">'+
                                '  <li>'+
                                ' <div class="drodown">'+
